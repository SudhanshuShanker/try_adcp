...
class Kamaji:
    ...
    def filterLigands(self):
        ...
        if 'unknown' not in self.structureTypes:
            return
        for resId in self.structureTypes['unknown']:
    ...
    def filterWaters(self):
        ...
        if not 'unknown' in self.structureTypes:
            return
        for resIdx in self.structureTypes['unknown']:
    ...
    def clusterGroups(self):
        ...
        if not 'unknown' in self.structureTypes:
            return
        clusters = {}
        for r1 in self.structureTypes['unknown']:
            clusters[r1] = []
            for r2 in self.structureTypes['unknown']:
                if r1 == r2: continue
    ...
    def filterAdditives(self):
        ...
        cutoffFP = 0.8
        cutoffMWratio = 0.5
        for resId, info in self.structureTypes['unknown']:
            res = self.mol.GetResidue(resId)
...
class MultiStatePDB:
    ...
    def parsePdbInfo(self):
        ...
        for l in self.getHeader():
            ...
            if l[0:6] == 'REMARK':
                value = l[0:10]
                if not value in self.remarkToType: 
                    value = None
                    continue
            elif l[0:6] == 'HET   ':
                value = l[0:6]
...
class KamajiInterface:
    ...
    def getBioPolymerInfo(self, data):
        """ """
        info = []
        info.append( ('residues', '%d' % len(data) ) )
        return info
    ...
    def getPeptideInfo(self, data):
        ...
        info.append( ('lenght' , len(data['data']) ) )
        return info
