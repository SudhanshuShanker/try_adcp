    ...
    def __init__(self, ligand, mapsFolder, receptor=None,
        ...
        *anchor*.npy in mapsFolder. Else if tpointsFilename starts with a path
        ...
        if covalentIndices is not None:
            ...
            covLigAnchorAt = [self.ligand._ag[int(al1.getIndices()[0])],
                              self.ligand._ag[int(al2.getIndices()[0])],
                              self.ligand._ag[int(al3.getIndices()[0])]]
        ...
        if FTRecsrc is not None:
            d = {}
            exec(compile(open(FTRecsrc).read(), FTRecsrc, 'exec'), d, d)
            self.RFTGen = d['rft']
        ...
        for mapType, mapFilename in list(mapFiles.items()):
            _map = GridMap()
