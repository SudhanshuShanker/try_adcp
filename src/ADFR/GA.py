...
class Population(list):
    ...
    def sort(self):
        ...
        print ("SORT 1111")
        list.sort(self, key=getscore, reverse=True)
        self.sorted = True
        print ("SCORE SORTED:")
        for v in self: print(v._score) 
...
class GA:
    ...
    def evolve(self, maxGens=1000, p_crossover=0.8, p_mutation=0.05,
        ...
        for m in self.pop[0].genomePy.ft.motions:
            print(m.getDeltaAmplitudeBase(), end=' ')
        print()
        ##
        ## Search Stats
        self.nbSWcalls = 0
    ...
    def step(self):
        ...
        if len(remainder)>2 and self.useClustering:
            ...
            for cnum, c in enumerate(clusters):
                ...
                # compute RMSD
                if self.verbose >= 2:
                    print("   %3d  %3d  %3d"%(cnum, len(c), rep), end=' ')
                ...
                if self.verbose >= 2:
                    print(" %12.3f %12.3f %12.3f %6.3f %s"%(-self.pop[rep]._real_score,\
                                                            -self.pop[rep]._real_score-self.pop[rep].energies['LL'], numpy.mean(scores), numpy.std(scores), cstr), end=' ')
        ...
        while len(gaind) < gaindSize:
                ...
                if self.verbose >= 2:
                    if o1star or o2star:
                        print('\n  (%3d, %3d) CRO O1: %12.3f -> %12.3f%1s (%4d) O2: %12.3f -> %12.3f%1s (%4d)'%(
                            ...
                            -sisScore, -sisnScore, o2star, nbSteps2), end=' ')
                    if o1starmin:
                        print('\n               SMI O1: %12.3f -> %12.3f%1s (%4d)'%(
                            -bronScore, -minbroScore, o1starmin, nbSteps1min), end=' ')
                    if o2starmin:
                        print('\n               SMI O2: %12.3f -> %12.3f%1s (%4d)'%(
                            -sisnScore, -minsisScore, o2starmin, nbSteps2min), end=' ')
                ...
                if self.verbose > 2:
                    if o1star1:
                        if not (o1star or o2star):
                            print('\n  (%3d, %3d)'%(p1Index, p2Index), end=' ')
                        print('\n MUT O1: %12.3f -> %12.3f -> %12.3f%1s (%4d)'%(
                            -a1, -c, -minbroScore, o1star1, nbStepsmut), end=' ')# self._totalStepInLastMinimize),
                    if o1star1min:
                        #if not (o1star or o2star):
                        #    print '\n  (%3d, %3d)'%(p1Index, p2Index),
                        print('\n SMI O1: %12.3f -> %12.3f -> %12.3f%1s (%4d)'%(
                            -c, -broScore,-minbroScore, o1star1min, nbStepsmutmin), end=' ')# self._totalStepInLastMinimize),
                ...
                if self.verbose > 2:
                    if o2star1:
                        ...
                        print('\n MUT O2: %12.3f -> %12.3f -> %12.3f%1s (%4d)'%(
                            -a, -c, -sisScore, o2star1, nbStepsmut), end=' ')# self._totalStepInLastMinimize),
                    if o2star1min:
                        #if not (o1star or o2star or o1star1):
                        #    print '\n  (%3d, %3d)'%(p1Index, p2Index),
                        print('\n SMI O2: %12.3f -> %12.3f -> %12.3f%1s (%4d)'%(
                            -c, -sisScore,-minsisScore, o2star1min, nbStepsmutmin), end=' ')# self._totalStepInLastMinimize),
            ...
            if len(newInds):
                ...
                newInds.sort(key=getscore, reverse=True)
        ...
        pop.sort(key=getscore, reverse=True)
        if sanityCheck:
