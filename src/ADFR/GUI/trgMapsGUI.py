...
class TrgMapsGui(QtGui.QDialog):
    ...
    def addTrg(self, filename=None, boxGeom=None):
        ...
        if trgname in self.trg_data and self.trg_data[trgname]['filename']==os.path.abspath(filename):
    ...
    def addMap(self, mapType, parent):
        ...
        if trgName not in self.iso_data:
    ...
    def mapTypeSelector_cb(self, item):
        ...
        for mt in added_maptypes:
            if mt in newDict:
                ...
                newDict[mt] = False
        for mt, val in list(newDict.items()):
            if val:
                self.addMap(mt, item)
            else:
    ...
    def removeMap(self, mapType, trgItem):
        for i in range(trgItem.childCount()):
            ...
            if mapItem.text(0) == mapType:
                ...
                if mapType in self.trg_data[trgName]:
                    self.trg_data[trgName].pop(mapType)
                if trgName in self.iso_data:
                    if mapType in self.iso_data[trgName]:
        ...
        if item and hasattr(item, "_object"):
            if isinstance(item._object, MapsFile):
                ...
                if reply == QtGui.QMessageBox.Yes:
                    ...
                    if trgName in self.iso_data:self.iso_data.pop(trgName) 
...
class MapTypeChooser(QtGui.QDialog):
    ...
    def cancel_cb(self):
        n = 0
        for atype, checked in list(self.mapList.items()):
            item = self.items[n]
            if checked:
                item.setCheckState(QtCore.Qt.Checked)
            else:
