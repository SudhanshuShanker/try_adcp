...
class ArgParser(ArgumentParser):
    ...
    def __init__(self, target, check=True, *arg, **kw):
        ...
        advancedGroup.add_argument(
            ...
            type=float, metavar=('2.5'), default=-1.0,
    ...
    def __init__(self, prog='agfr', *args, **kw):
        ...
        if prog == "agfr":
            ...
            group.add_argument('-r', "--receptor", dest="receptorFile",
                          ...
                          help="receptor for which to compute maps (in PDBQT or PDB format)")
...
def makeConfigFile(options, filename, cmd="agfr"):
    ...
    notIncluded = []
    for option, val in list(options.items()):
        if option in parserOptions:
            ...
            #    valstr = val #option values must be strings (python 3)
            else: valstr = str(val)
