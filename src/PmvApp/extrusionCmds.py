...
##                     return  sheet2DRes, sheetCoords, isHelix
    ...
    def doit(self, nodes, sheet2DName, ctlAtmName, torsAtmName,
        ...
        for chain in chains:
            ...
            if sheet2DName not in chain.sheet2D:
...
class Nucleic_Acids_properties(MVCommand):
    ...
    def checkArguments(self, color_A=[1,0,0], color_G=[0,0,1], color_T=[0,1,0],
        ...
        assert isinstance (scale_purine, Number)
        assert isinstance (scale_pyrimidine, Number)
        assert isinstance (height_purine, Number)
        assert isinstance (height_pyrimidine, Number)
        kw = {}
        kw['color_A'] = color_A
        kw['color_G'] = color_G
        kw['color_C'] = color_C
