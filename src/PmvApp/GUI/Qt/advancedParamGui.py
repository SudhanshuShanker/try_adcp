...
class CustomizeWidget(QtGui.QWidget):
    ...
    def __init__(self, name, molSel, selName, perAtomTabs=["Properties", "Colors"],
        ...
        for sel in self.selection:
            ...
            if self.geomName not in mol._renderingProp:
    ...
    def setOpacity(self, val):
        for sel in self.selection:
            mol =  sel.getAtomGroup().getMolecule()
            inds = sel.getIndices()
            if sel not in self.initialValues:
            ...
            if mol._ag.getData(dataLabel) is None:
                ...
                if "opacity" not in self.initialValues[sel]:
    ...
    def saveCurrentGeomColor(self):
        ...
        for sel in self.selection:
            ...
            if sel not in self.initialValues:
                self.initialValues[sel] = {}
            if 'colors' not in self.initialValues[sel]:
...
class IndexedGeomCustomizeWidget(CustomizeWidget):
    ...
    def updateGlobalProp(self, kw, geomName=None):
        ...
        for sel in self.selection:
            ...
            if sel not in self.initialValues:
                self.initialValues[sel] = {}
            for propName, value in list(kw.items()):
                if propName not in self.initialValues[sel]:
                    try:
                        if isinstance(prop[propName], str) and prop[propName] in self.displayCmd._renderToPmv:
    ...
    def setStroke(self, val, strokeWidth):
        ...
        if val:
            ...
            if sel in self.initialValues:
                if "backfaceColor" in self.initialValues[sel]:
                    setColor = False
            if setColor:
...
class LinesCustomizeWidget(CustomizeWidget):
    ...
    def cancel(self):
        ...
        for sel, molkw in list(self.initialValues.items()):
    ...
    def updateGlobalProp(self, propName, value):
        for sel in self.selection:
            mol =  sel.getAtomGroup().getMolecule()
            if mol not in self.initialValues:
                self.initialValues[mol] = {}
            if propName not in self.initialValues[mol]:
...
class CpkCustomizeWidget(IndexedGeomCustomizeWidget):
    ...
    def setCPKRadius(self, size, prop=""):
        for sel in self.selection:
            ...
            if sel not in self.initialValues:
                self.initialValues[sel] = {}
            if "cpk_radius" not in self.initialValues[sel]:
...
class SBCustomizeWidget(IndexedGeomCustomizeWidget):
    ...
    def setBallRadius(self, size, prop=""):
        ...
        for sel in self.selection:
            ...
            if sel not in self.initialValues:
                self.initialValues[sel] = {}
            if 'balls_radius' not in self.initialValues[sel]:
    ...
    def setCylRadius(self, rad):
        for sel in self.selection:
            ...
            if sel not in self.initialValues:
                self.initialValues[sel] = {}
            if 'cyl_radius' not in self.initialValues[sel]:
...
class MsmsCustomizeWidget(IndexedGeomCustomizeWidget):
    ...
    def getSurfName(self, sel):
        ...
        names = []
        for name in list(gc.atoms.keys()):
            if name.find("msms") >= 0:
                if name in gc.geoms:
    ...
    def setProbeRadius(self, probeRad):
        for sel in self.selection:
            ...
            if sel not in self.initialValues:
                self.initialValues[sel]={}
            for name in names:
                if name not in self.initialValues[sel]:
                    self.initialValues[sel][name] = {}
                if 'probeRadius' not in self.initialValues[sel][name]:
    ...
    def setQuality(self, dens):
        for sel in self.selection:
            ...
            for name in names:
                if name not in self.initialValues[sel]:
                    self.initialValues[sel][name] = {}
                if 'quality' not in self.initialValues[sel][name]:
    ...
    def updateGlobalProp(self, kw):
        for sel in self.selection:
            ...
            for name in names:
                ...
                for propName, value in list(kw.items()):
                    if propName not in self.initialValues[sel][name]:
                        try:
                            if isinstance(prop[propName], str) and prop[propName] in self.displayCmd._renderToPmv:
    ...
    def setPropInitValues(self, sel, name):
        if name not in self.initialValues[sel]:
            self.initialValues[sel][name] = {}
        if 'radii' not in self.initialValues[sel][name]:
            mol = sel.getAtomGroup().getMolecule()
    ...
    def setStroke(self, val, strokeWidth):
        ...
        if val:
            ...
            if sel in self.initialValues:
                ...
                if name in self.initialValues[sel]:
                    if "backfaceColor" in self.initialValues[sel][name]:
                        setColor = False
            if setColor:
                # initial stroke color is black
                kw["backfaceColor"]=[0.0, 0.0, 0.0, 1.0]
    ...
    def saveCurrentGeomColor(self):
        ...
        for sel in self.selection:
            ...
            for name in names:
                ...
                if name not in self.initialValues[sel]:
                    self.initialValues[sel][name] = {}
                if 'colors' not in self.initialValues[sel][name]:
    ...
    def setOpacity(self, val):
        for sel in self.selection:
            ...
            for name in names:
                ...
                if mol._ag.getData(dataLabel) is None:
                    opacity = numpy.ones(len(mol._ag), 'f')
                    if "opacity" not in self.initialValues[sel][name]:
...
class  CartoonCustomizeWidget(IndexedGeomCustomizeWidget):
    ...
    def setPropInitValues(self, sel):
        if sel not in self.initialValues:
            self.initialValues[sel] = {}
        if 'shape' not in self.initialValues[sel]:
            mol = sel.getAtomGroup().getMolecule()
...
class AtomLabelsCustomoizeWidget(CustomizeWidget):
    ...
    def cancel(self):
        for sel, kw in list(self.initialValues.items()):
            mol =  sel.getAtomGroup().getMolecule()
            if "colors" in kw:
                colors, inds = kw.pop("colors")
                mol._ag.setData('colorsIndices_%s'%self.geomName, inds)
...
class CustomizeRepresentationsDialog(QtGui.QDialog):
    ...
    def getGeoms(self, obj, visibleOnly=True):
        ...
        for mol, sele in moleculesAndSelections:
            gc = mol.geomContainer
            for gname, ats in list(gc.atoms.items()):
                name = None
                if gname in gc.geoms:
    ...
    def createTabWidgets(self):
        ...
        surfName = None
        for name, selection in list(geoms.items()):
            widgetClass = self.tabWidgetClass.get(name, None)
            if widgetClass:
                if name not in self.pageWidgetsDict:
                    ...
                    if not(len(items)):
                        ...
                        QtGui.QListWidgetItem(name, self.contentsWidget)
        for name in list(self.pageWidgetsDict.keys()):
            if name not in list(geoms.keys()):
    ...
    def cancel_cb(self):
        ...
        for widget in list(self.pageWidgetsDict.values()):
            widget.cancel()
