...
class Dashboard(QtGui.QTreeWidget):
    ...
    def deleteObjectEventHandler(self, event):
        if event.objectType=='Molecule':
            mol = event.object
            self.invisibleRootItem().removeChild(list(mol._treeItems.keys())[0])
    ...
    def deleteAtomEventHandler(self, event):
        ...
        for root in list(mol._treeItems.keys()):
    ...
    def addAtomEventHandler(self, event):
        ...
        for root in list(mol._treeItems.keys()):
            ...
            if not hasattr(root, 'dummyChild'):
                for n in range(root.childCount(), 0, -1):
    ...
    def _selectionEventHandler(self, selection, setOn, setOff):
        ...
        if app.curSelection.nbAtoms()==0:
            ...
            if hasattr(app.activeSelection,'_treeItems') and \
                ...
                item = list(app.activeSelection._treeItems.keys())[0]
        ...
        ## selection is NOT empty
            ...
            if self.pmvGUI.activeSelection is None:
                ...
                if hasattr(app.curSelection, '_treeItems'):
                    self.setCurrentSelection(list(app.curSelection._treeItems.keys())[0])
                else:
                    ...
                    for gname, ats in list(gc.atoms.items()):
                        if gname in gc.geoms:
                            if gname in gc.geomCmds:
                                ...
                                if cmd: cmds[cmd] = sel
                    for cmd, sel in list(cmds.items()):
                        cmd(sel)
    ...
    def setCurrentSelection(self, item):
        ...
        if item:
            ...
            if hasattr(self.pmvGUI.activeSelection, '_treeItems'):
                list(self.pmvGUI.activeSelection._treeItems.keys())[0].setBackground(0, self.colors['white'])
        ...
        for sel in selections:
            ...
            if hasattr(self, '_alt'):
                ...
                ## multi conformations
                    ...
                    if mol._multi:
                        self.multiGoto(mol, mol.curMolIndex()-1, list(mol._treeItems.keys())[0])
                elif e.key()==QtCore.Qt.Key_Right: # PREVIOUS
                    if mol._multi:
                        self.multiGoto(mol, mol.curMolIndex()+1, list(mol._treeItems.keys())[0])
                elif e.key()==QtCore.Qt.Key_Home: # FIRST
                    if mol._multi:
                        self.multiGoto(mol, 0, list(mol._treeItems.keys())[0])
                elif e.key()==QtCore.Qt.Key_End: # LAST
                    if mol._multi:
                        self.multiGoto(mol, mol.numMols()-1, list(mol._treeItems.keys())[0])
