...
class MolApp(AppFramework):
    ...
    def handleAddAtoms(self, event):
        ...
        for atomset in list(gc.atoms.values()):
            atomset._ag = mol._ag
        coords = mol._ag.getCoords().astype('f')
        # fix geometry vertices
        for geom in list(gc.geoms.values()):
    ...
    def __init__(self, title="Molecule Viewer", eventHandler=None):
        ...
        ## selections
        ##
        self.curSelection = PmvSelection(self, 'Current Selection')
...
of view when a new molecule is loaded""")
    ...
    def multiGoto(self, mol, idx, item=None):
        ...
        if mol._multi=='molecules':
            ...
            for mol in mols:
                ...
                if 'aromaticSpheres' in mol.geomContainer.geoms:
                ...
                if hasattr(mol, "_msmsData"):
                    for name in list(mol._msmsData['msms'].keys()):
                ...
                if hasattr(mol, "_cartoonData"):
                    ...
                    if cartoonData is None:
                        ...
                        for chid, data in list(cartoonData.items()):
    ...
    def handleDeleteAtomsEvent(self, event):
        ...
        if self.curSelection.isAnySelected(atSet):
            ...
            for ag, sel in list(self.curSelection._agDict.items()):
                if sel == selection:
        ...
        for name, sel in list(self.namedSelections.items()):
...
"""
        ...
        if isinstance(obj, str):
            ...
            elif obj in list(self.namedSelections.keys()):
                obj = self.namedSelections[obj]
            elif obj in list(self.groups.keys()):
                obj = self.groups[obj]
            else:
        ...
        if isinstance(obj, SelectionSet):
            ...
            if name=='Current Selection' or name in list(self.namedSelections.keys()):
            ...
            if obj == self.curSelection:
                ...
                def mkSelCurSel(selection, item=None):
                    ...
                    selection.name = 'Current Selection'
            ...
            if obj.name == name: return
            if name in list(self.groups.keys()):
    ...
    def createGroup(self, name, group=None):
        assert name not in list(self.groups.keys())
        grp = Group(name)
...
provided by obj
    ...
    def getGeoms(self, obj, visibleOnly=True):
        ...
        for mol, sele in moleculesAndSelections:
            gc = mol.geomContainer
            for name, ats in list(gc.atoms.items()):
                if name in gc.geoms:
...
"""
        ...
        if self.gui:
            if group:
                if isinstance(group, str):
                    ...
                    assert isinstance(group, Group)
                    parent = list(group._treeItems.keys())[0]
                group.add(selSet)
            else:
                parent = None
    ...
    def saveFullSession(self, name, inlcudeMultiMol):
        ...
        for mol in self.Mols:
            ...
            list(mol.geomContainer.atoms.keys())
            setDict = {}
        ...
        with open('selections.npz', 'wb') as f:
            ...
            for name, selSet in list(self.namedSelections.items()):
                for sel in selSet:
    ...
    def readFullSession(self, name):
        ...
        if True:
            ...
            with open('selections.npz') as f:
                ...
                while True:
                    ...
                    if selname=='CURRENT':
                        ...
                        if selname in selections:
                            selections[selname].append(sel)
                        else:
                            selections[selname] = [sel]

            lastSele = None
            for name, sels in list(selections.items()):
            ...
            exec(compile(open('./session.py').read(), './session.py', 'exec'), {'self':self})
            os.chdir(cwd)
    ...
    def getStateCodeForMolecule(self, mol):
        ...
        for key in list(mol._ag._data.keys()):
        ...
        ## Lines
        ##
        if "singleBonds" in gca and len(gca["singleBonds"]):
            ...
            if mol._aromaticArcs is not None and len(mol._aromaticArcs):
                ...
                for index in list(mol._aromaticArcs.keys()):
        ...
        ##
        if hasattr(gc, "msms"):
            for name, srfa in list(gc.msms.items()):
                srf = srfa[0]
                atoms = gca[name]
        ...
        if hasattr(gc, "boundGeom"):
            for name in list(gc.boundGeom.keys()):
        ...
        ##
        data={}
        if 'sticks' in gca and gca['sticks']:
            # sticks radius: cRad
    ...
    def getStateCodeForSelection(self):
        ...
        if len(self.namedSelections):
            for sname, sele in list(self.namedSelections.items()):
                if lastSele is not None:
    ...
    def readFullSessionOLD(self, name):
        ...
        try:
            # execute session.py file inside untar'ed folder
            exec(compile(open('./session.py').read(), './session.py', 'exec'))
            os.chdir(cwd)
    ...
    def bindGeomToMolecularFragment(self, geom, sel, cutoff_from=3.5,
        ...
        try:
            ...
            if len(cl_atoms):
                ...
                ids = sel.getIndices()
                mol_lookup = dict(list(zip(ids, list(range(len(ids))))))
    ...
    def focusScene(self, obj=None, viewer=None, duration=0.5, padding=10):
        ...
        if len(coords):
            ...
            p = Points( "selatoms", vertices=coords.astype("f"),
                        faces=[list(range(len(coords)))], visible=1,
...
class MolGeomContainer(GeomContainer):
    ...
    def addGeom(self, geom, parent=None, redo=False, makeAtomSet=True,
        ...
        if makeAtomSet:
            if geom.name not in self.atoms:
