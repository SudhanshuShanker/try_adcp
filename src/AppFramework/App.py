...
"""
import sys, os, _thread, weakref, traceback
...
class AppFramework:
    ...
    def __init__(self, name='MyApp', eventHandler=None, errorManager=None,
        ...
        self.objectsLock = _thread.allocate_lock()

        # lock needs to be acquired before topcommands can be run
        self.commandsLock = _thread.allocate_lock()
    ...
    """
        ...
        for cmdName in commands:
            ...
            if cmdName in self.commands: # already loaded
                continue
            # REPORT LOADING
            ...
            if cmdName in mod.commandsInfo['icoms']:
                lClass = ICmdLoader
            else:
                lClass =  CmdLoader
    ...
    def addCommand(self, command, name):
        ...
        if name in list(self.commands.keys()):
    ...
    def addObjectType(self, typeName, typeValidator, events):
        ...
        assert typeName not in self.objectTypes
    ...
    def setOnAddObjectCmd(self, objType, cmds, argsList=None, kwList=None):
        ...
        assert objType in self.objectTypes, "bad objecttype %s"%objType
    ...
    def source(self, filename, globalNames=1, globDict={}):
        ...
        try:
            ...
            exec(compile(open( filename).read(), filename, 'exec'), globDict)#, localDict)
        finally:
...
but will be used a new surfaces are created)""", category="Graphics")
    ...
    def addColorMap(self, colorMap):
        ...
        if 'colorMap.name' in self.colorMaps:
    ...
    def loadColormap(self, filename=None):
        ...
        if newColorMap:
            ...
            for k in list(self.colorMaps.keys()):
                if k==name:
...
class GeomContainer:
    ...
    def addGeom(self, geom, parent=None, redo=False):
        ...
        if nameUsed or geomName in self.geoms:
