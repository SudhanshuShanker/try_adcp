...
def getFlexResNames():
    ...
    return s1.intersection(list(rotlib.angleList.keys()))
...
class runAGFR:
    ...
    def processReceptor(self, atoms, addH=True, charges={}):
        ...
        for key, charge in list(charges.items()):
            chid, resname, resnum, icode = key.split(':')
        ...
        if addH:
            ...
            for selstr, charge in list(charges.items()):
    ...
    def loadReceptor(self, filename, processReceptor=True):
        ...
        if len(self.PDBprofile._assemblies)==1:
            ...
            name = list(self.PDBprofile._assemblies.keys())[0]
            ...
            if self._bioMolNum == "ASU":
                for key in list(self.PDBprofile._assemblies.keys()):
                ...
                for key in ["Biomol %s" % self._bioMolNum,
                            ...
                            "%s" % self._bioMolNum]:
                    if key in self.PDBprofile._assemblies:
                        name = key

            if name is not None:
                self.PDBprofile._currentBM = self.PDBprofile._assemblies[name][1]
            else:
                raise RuntimeError("ERROR: requested Biomol %s is not known. Biomolecules in pdb file: %s. Using %s" % (self._bioMolNum, list(self.PDBprofile._assemblies.keys()),  list(self.PDBprofile._assemblies.keys())[0]))
    ...
    def setMapTypes(self, mapTypes):
        if mapTypes=='all':
            ...
            self.atypes = list(self.ADAtomTypes.keys())
    ...
    def afterAutoSite2(self, gc, spacing=1.0, cutoff=10, ligandSize=500, pepScore=False, verbose=False):
        ...
        if nbp < 50:
            ...
            dcl._clusters = [list(range(len(gc._indices)))]
    ...
    def checkFlexResidues(self, flexResStr):
        ...
        if len(outsideRes):
            outsideResStr = ""
            for k, v in list(outsideRes.items()):
    ...
    def checkCovBondParams(self):
        err = []
        if 'covalentBond' not in self.data:
    ...
    def checkComputeGrids(self):
        ...
        # Check if the moving atoms of flex residues are inside the box
        if 'flexResStr' in self.data and self.data['flexResStr']:
