...
class AtomTypeSelector(QtGui.QDialog):
    ...
    def cancel_cb(self):
        n = 0
        for atype, checked in list(self.atomList.items()):
            item = self.items[n]
            if checked:
                item.setCheckState(QtCore.Qt.Checked)
            else:
...
class CovalentLigAtomsList(QtGui.QDialog):
    ...
    def getAtomList(self):
        ...
        for i in range(self.resListWidget.count()):
            ...
            txtList = txt.split(":")
            chid = txtList[0]
            if chid not in resDict:
                resDict[chid] = txtList[1]
            else:
                resDict[chid] += ","+txtList[1]
        resStr = ""
        for chid, res in list(resDict.items()):
...
class GridGUI(QtGui.QWidget):
    ...
    def setAppTitle(self):
        rname = lname = 'None'
        if self.agfr.PDBprofile:
            app = 'PDBprofiler'
            rname = self.agfr.PDBprofile._pdbid
        else:
            app = 'AGFR'
            if self.receptor:
    ...
    def selectionFromResString(self, resStrList):
        ...
        for resStr in resStrList:
            ...
            if chid not in chDict:
                chDict[chid] = {}
            if resstr not in chDict[chid]:
                chDict[chid][resstr] = ""
            atnames = ""
            for at in atoms.replace(',', ' ').split():
                atnames += at+ " "
            chDict[chid][resstr]+=atnames
        for chid in  list(chDict.keys()):
            for resstr in list(chDict[chid].keys()):
    ...
    def removeLigAtom(self, atInd, excluded):
        ...
        list(map(newExcluded.extend, self._newExcluded))
        if len(newExcluded):
    ...
    def toggleRotatableBond(self, selections, operation):
        ...
        if key not in self._frozenBonds: return
    ...
    def createPyShellWidget(self):
        ...
        if not use_ipython_shell:        
            ...
            from PmvApp.GUI.Qt.pyshell import PyShell
...
#            kernel.shell
    ...
    def getReceptorMaps(self, filename):
        ...
        for mapFileName in zf.namelist():
            w = mapFileName.split('.')
            if w[-1]=='map':
                if w[-2] in self.agfr.ADAtomTypes:
                    atomTypes += '%s '%w[-2]
                    nb += 1
    ...
    def addGeomsForGaps(self, gapsPerCh):
        ...
        if not mol :return
        for chid, chgaps in list(gapsPerCh.items()):
    ...
    def fetchPDB(self, name=None):
        if name is None:
            from PmvApp.GUI.Qt.PmvGUI import FetchGUI
    ...
    def getReceptorFilename(self):
        filename, selfilter = QtGui.QFileDialog.getOpenFileName(
            ...
            self.tr("receptor files (*.pdbqt *.pdb *.ent *.ent.gz);; All files (*)"))
        if filename:
            #self.loadReceptor(filename.encode('ascii', 'replace'))
            self.loadReceptor(filename)
    ...
    def loadReceptor(self, filename):
        ...
        if self.agfr._pdbInput:
            ...
            if 'Biomol 1' in tw:
                ...
                #tw.values()[0].buildUI()
                self.analyserWidget.treesNoteBook.setCurrentWidget(list(tw.values())[0])
    ...
    def finalizeMolecules(self):
        ...
        if len(self.PDBprofile._sites):
            for k,v in list(self.PDBprofile._sites.items()):
        ...
        self._mutatedResidues = {}
        for key, resnames in list(self.PDBprofile._resToMutate.items()):
    ...
    def openTrgMapGui(self):
        ...
        if self.showTrgGuiAction.isChecked():
            ...
            if not self.trgMapGui:
                ...
                trgInGui = [val['filename'] for val in list(self.trgMapGui.trg_data.values())]
        ...
        if self._ligandMultiMol:
            if isinstance(self._ligandMultiMol, MultiMolecule):
                ...
                if ok:
                    ...
                    if 'aromaticSpheres' in self.ligand.geomContainer.geoms:
    ...
    def pickATypes(self):
        ...
        for at in list(self.agfr.ADAtomTypes.keys()):
            if at in atstr:
                d[at] = True
            else:
                d[at] = False
...
##    @waiting_effects
    ...
    def computeGrids(self):
        ...
        filename, filtr = QtGui.QFileDialog.getSaveFileName(
            self, "Target file name", recName+'.trg', "Target file (*.trg)")
        if not filename: return
        fi = QtCore.QFileInfo(filename)
        self.destinationFolder = fi.completeBaseName()# This returns a string
