...
class ResiduesTree(QtGui.QWidget):
    ...
    def buildUI(self):
        ...
        if self._altloc is not None:
            altloc = []
            for chid, reslist in list(self._altloc.items()):
                for resname, resnum, icode in reslist:
                    resKey = '%s:%s%s%s'%(chid,resname,resnum,icode)
                    if resKey in self._resToItem: altloc.append(resKey)
    ...
    def buildTree(self):
        ...
        if self._sites:
            ...
            for sitename, residues in list(self._sites.items()):
    ...
    def selectClose(self):
        ...
        if len(atoms):
            ...
            if self.checkedItems:
                ...
                for ind in indices:
                    if '%s:%d%s'%(chids[ind],
                                                            resnums[ind],icodes[ind]) in self.checkedItems:
            ...
            # uncheck all
            for item in list(self._resToItem.values()):
