...
class ComputeSecondaryStructureCommand(MVCommand):
    ...
    def onAddCmdToApp(self):
        ...
        # application
        if 'saveSet' not in self.app().commands:
    ...
    def checkArguments(self, nodes, molModes=None):
        ...
        if molModes:
            ...
            for mode in list(molModes.values()):
                if mode:
        ...
        for mol in molecules:
            try:
                ...
                if not molModes:
                    ...
                    if molModes and mol.name not in molModes:
...
#                                    )
    ...
    def clean(self, mol):
        ...
        for c in mol.chains:
            if hasattr(c, 'sheet2D') and 'ssSheet2D' in c.sheet2D:
...
class ExtrudeSecondaryStructureCommand(MVCommand):
   ...
   Optional Arguments:\n    
    ...
    def onAddObjectToViewer(self, obj):
        ...
        # have been created.
        obj.__hasSSGeom = 0
        if 'dashboard' in self.app().commands:
    ...
    def createGeometries(self, obj):
        ...
        if 'secondarystructure' not in geomC.geoms:
    ...
    def checkArguments(self, nodes, shape1=None, shape2=None, frontcap=True,
        ...
        assert isinstance (larrow, Number)
    ...
    def doit(self, nodes, shape1=None, shape2=None, frontcap=True, endcap=True,
        ...
        selMols, selResidues = self.app().getNodesByMolecule(self.app().activeSelection.get(),
                                                          Residue)
        molSelectedResiduesDict = dict( list(zip( selMols, selResidues)) )
        ...
        for mol, residues in map(None, molecules, residueSets):
            try:
                ...
                # highlight selection
                if mol in molSelectedResiduesDict and len(molSelectedResiduesDict[mol]) > 0:
                    lHighlight = True
                else:
                    lHighlight = False
                ...
                for i in range(len(chains)):
                    ...
                    elif 'ssSheet2D' not in chain.sheet2D:
                        rt = chain.ribbonType()
                        if rt=='NA':
                            ExtrudeNA(chain)
                            newsheet = 1
...
"""
        ...
        for mol, atomSet in zip(molecules, ats):
            ...
            if 'secondarystructure' not in mol.geomContainer.geoms: continue
            ...
            try:
                ...
                for c in mol.chains:
                    ...
                    if not hasattr(c, 'sheet2D'):
                        ...
                        continue
                    elif ('ssSheet2D' in c.sheet2D and \
                          c.sheet2D['ssSheet2D'] is None): continue
                    if 'SS' in mol.geomContainer.geoms :
                        faces=[]
                        vertices=[]	
                        normals=[]							
                        name = "SS"+c.id
                    ...
                    for s in range(len(c.secondarystructureset)):
                ...
                if 'secondarystructure' in mol.geomContainer.geoms:
...
class UndisplayExtrudedSSCommand(DisplayCommand):
    ...
    """
    def onAddCmdToApp(self):
        if 'displayExtrudedSS' not in self.app().commands:
            self.app().lazyLoad('secondaryStructureCmds',
...
class ColorBySSElementType(ColorFromPalette):
    ...
    def onAddCmdToApp(self):
        ...
        if 'color' not in self.app().commands:
        ...
        for mol in self.molSet:
            try:
                for gName in geomsToColor:
                    if gName not in mol.geomContainer.geoms: continue
