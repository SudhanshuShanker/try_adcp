...
class ColorRainbowChain(ColorByMolecule):
    ...
    def getColors(self, sel):
        ...
        for chain in mol._ag.iterChains():
            indices = list(range(chain.numAtoms()))
...
class SetOpacity(MVCommand):
    ...
    def getAvailableGeoms(self, selections, showUndisplay=0):
        ...
        for selection in selections:
            ...
            for geomName, geom in list(geomC.geoms.items()):
        ...
        return list(geomsAvailable.keys())
    ...
    def checkArguments(self, selection, opacities, geoms=['all']):
        ...
        for geomName in geoms:
            assert geomName in list(mol.geomContainer.geoms.keys())
    ...
    def doit(self, nodes, geomsToColor=['all']):
        ...
        for m in self.molSet:
            ...
            for g in geomsToColor:
                try:
                    ge = geomc.geoms[g]
                    colors = self.palette.lookup(list(range(len(ge.instanceMatricesFortran))))
...
## """)
    ...
    def checkArguments(self, nodes, geomsToColor, property,
        ...
        if mini is not None:
            assert isinstance (mini, Number)
        if maxi is not None:
            assert isinstance (maxi, Number)
            assert maxi >= mini
        if isinstance (colormap, str):
            assert colormap in self.app().colorMaps
        else:
