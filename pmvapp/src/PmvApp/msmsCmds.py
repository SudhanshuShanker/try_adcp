...
class ComputeMSMS(MVCommand): #, MVAtomICOM):
    ...
    def doit(self, molSel, surfName=None, probeRadius=None, density=None,
        ...
        if surfName in mol._msmsData['msms']:
        ...
        indexMap[indices] = list(range(len(sel)))
        ...
        if radii is None:
            ...
            assert len(radii)==len(sel) and isinstance(radii[0], Real)
            atmRadii = radii
        ...
        if surfName not in mol._msmsData['msms']:
...
class DisplayMSMS(DisplayIndexedGeomCommand):
    ...
    def doit(self, molSel, **kw):
        ...
        if surfNames is None:
            ...
            surfNames = list(mol._msmsData['msms'].keys())
    ...
    def refreshDisplay(self, mol, names=None):
        ...
        if names is None:            
            names = list(mol._msmsData['msms'].keys())
...
class UndisplayMSMS(DisplayMSMS):
    ...
    def onAddCmdToApp(self):
        ...
        if 'displayMSMS' not in self.app().commands:
