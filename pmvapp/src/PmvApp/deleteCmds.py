...
class DeleteMolecule(MVCommand):
    ...
    def doit(self, molSel):
        ...
        for name, selSet in list(self.app().namedSelections.items()):
    ...
    def updateUndoRedoCmdStack(self, molecule, command):
        ...
        for i, cmdlist in enumerate(command.cmdStack):
            ...
            ncmds = len(cmdlist[0])
            for j in range(len(cmdlist[0])-1, -1, -1): # loop backwards over the list of commands in undo/redo action 
...
class DeleteAtoms(MVCommand):
    ...
    def onAddCmdToApp(self):
        if 'deleteMolecule' not in self.app().commands:
