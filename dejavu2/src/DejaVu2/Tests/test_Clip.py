...
#
#
#  $Id: test_Clip.py,v 1.1.1.1.4.1 2017/07/13 22:24:00 annao Exp $
#
#
...
class Clip_ObjectTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if not hasattr(self, 'vi'):
            ...
            self.spheres = Spheres('test', centers=((-2.5,0,0),(4.5,0,0),),
                                   quality=15, radii=(2,4))
...
"""
        ...
        import sys, os
        if sys.platform == "darwin" and os.uname()[-1] == "x86_64":
            if sum(zbuf) == 0:
                print("clipping plane on Darwin x86_64 does not affect Z buffer")
                return
        zbuf.shape = (width, height)
        self.camera.master.master.lift()
        self.vi.OneRedraw()
        #check that something is in the right half
        midway = int(self.camera.width/2.)
        #self.assertEqual(round(zbuf[:,midway:].min(),1)< .5, True)
        self.assertEqual(zbuf[:,midway:].min() < .6, True)
...
overlap the camera's window
"""
        mat = rotax( (0,0,0), (0,0,1), math.pi/2.)
        self.spheres.AddClipPlane(self.clip)
        self.clip.Set(enabled=True)
        ...
        midway = int(self.camera.height/2.)
        #self.assertEqual(round(min(min(zbuf[midway:, :]))), 1.0)
        self.assertEqual(round(zbuf[midway:, :].min()), 1.0)
