...
from . import viewerConst, jitter
...
"""
    ...
    def printInfo(self):
        ...
        for key, values in list(self.hits.items()):
            print(key.name, end=' ')
            for v in values:
                print('vertex:%d instance:%s'%(v[0], str(v[1])), end=' ')
            print()
...
##             self.direction = self.lookAt - self.lookFrom
        ...
        if not val is None:
            assert len(val)==3
            assert type(val[0])==float
            assert type(val[1])==float
            assert type(val[2])==float
...
"""
        ...
        # skip object if it is in list of objects not shown in this Camera
        if self in obj.hiddenInCamera: return
...
#        gl.glPushAttrib(GL_LIGHTING_BIT)
        ...
        if self.drawMode == 2:
            ...
            for m in obj.instanceMatricesFortran:
                ...
                glPushMatrix()
                glMultMatrixf(m)
                for child in obj.children: self.Draw(child)
                glPopMatrix()
...
#                         GL_POLYGON_BIT)
            ...
            if len(self.viewer.noDpyListOpaque):
                ...
                for m in obj.instanceMatricesFortran:
                    glPushMatrix()
                    glMultMatrixf(m)
                    for child in self.viewer.noDpyListOpaque: self.Draw(child)
                    glPopMatrix()
            ...
            if len(self.viewer.noDpyListTransp):
                ...
                for m in obj.instanceMatricesFortran:
                    glPushMatrix()
                    glMultMatrixf(m)
                    for child in self.viewer.noDpyListTransp: self.Draw(child)
                    glPopMatrix()
                self.drawTransparentObjects = 0
