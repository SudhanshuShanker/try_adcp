    ...
    for p in ['.']+sys.path:
        ...
        for f in files:
            ...
            if os.path.exists( os.path.join( pdir, '__init__.py')) :
                ...
                flagline =[x for x in Lines if x.startswith("packageContainsVFCommands")]
                if not flagline ==[]:
            ...
            for i, (cmd, args, kw) in enumerate(cmdList):
                ...
                if hasattr(cmd, 'name'):
                    ...
                    if hasattr(cmd, "im_class"):
                        name = "%s.%s" % (cmd.__self__.__class__, cmd.__name__)
                    else:
                        name = cmd.__name__
...
#            self.app().removableCommands.saveAllSettings()
        ...
        if commands is None:
            ...
            if hasattr(mod,"initModule"):
                ...
                for name, values in list(mod.commandClassFromName.items()):
                    cmd = values[0]()
...
class loadModuleCommand(AppCommand):
    ...
    def loadModules(self, package, library=None):
        ...
        if package not in self.filenames:
        ...
        for key, value in list(self.filenames[pack].items()):
            pathPack = key.split(os.path.sep)
            if pathPack[-1] == package:
                newModName = [x[:-3] for x in value]
                ...
                newModName = ["%s.%s"%(prefix, x[:-3]) for x in value]
                #for mname in newModName:
                   #if not modulename has Command in it delete from the
                   #modules list
                   #if "Command" not in mname :
