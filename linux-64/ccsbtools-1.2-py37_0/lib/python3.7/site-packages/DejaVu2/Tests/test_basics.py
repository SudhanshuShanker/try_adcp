...
class basics_Tkinter_Viewer_Tests(unittest.TestCase):
    """
tests for Tkinter basics in DejaVu2.Viewer
    """
    def setUp(self):
        self.root = Tk()
        self.root.withdraw()
    ...
    def test_01_constructViewerWhenTkDefaultRootExists(self):
        ...
        """
        # this test would fail if DejaVu2.Viewer would try to create independent Tk
...
class basics_Viewer_Tests(unittest.TestCase):
    """
    tests for basics in DejaVu2.Viewer
    """
    ...
    def test_multipleViewers(self):
        ...
        s1 = Spheres('sph', vertices = [[0,0,0], [3,0,0], [0,5,0], [0,0,3]],
                 materials = ((1,0,0),), inheritMaterial=0)
        self.vi.AddObject(s1)
        vi2 = Viewer(verbose=0)
        s2 = Spheres('sph', vertices = [[0,0,0], [3,0,0], [0,3,0], [0,0,3]],
                 materials = ((0,1,0),), inheritMaterial=0)
        vi2.AddObject(s2)
        vi3 = Viewer(verbose=0)
        s3 = Spheres('sph', vertices = [[0,0,0], [3,0,0], [0,3,0], [0,0,3]],
                 materials = ((0,0,1),), inheritMaterial=0)
        vi3.AddObject(s3)
        vi2.Exit()
        vi3.Exit()
