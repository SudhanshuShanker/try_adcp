...
class UTVolRenGeom(VolumeGeom):
    ...
    def __init__(self, name=None, **kw):
        ...
        VolumeGeom.__init__(*(self,name,0), **kw)
	self.inheritXform=1
...
##  	self.InitVolumeRenderer()
    ...
    def grayRamp(self):
        self.byte_map = numpy.transpose(numpy.array([list(range(256)),list(range(256)),list(range(256)),list(range(256))] )).astype(numpy.uint8)
	self.volume.uploadColorMap(self.byte_map)
	
    def Set(self, check=1, redo=1, updateOwnGui=True, **kw):
	redoFlags = VolumeGeom.Set(*(self,0,0), **kw)
	s = kw.get('volscale')
