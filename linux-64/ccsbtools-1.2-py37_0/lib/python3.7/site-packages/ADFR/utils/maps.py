...
class MapsFile:
    ...
    def printInfo(self, indent=''):
        ...
        if 'covalentBond' in d and d['covalentBond'] is not None:
        ...
        if 'mapGradients' in d and d['mapGradients']:
            print(indent+'    gradients: Yes, ', end=' ')
            if d['gradCutOff']==-1:
    ...
    def getData(self):
        ...
        if self._data:
            ...
            with open(os.path.join(self.folder, 'data.pkl'), 'rb') as f:
...
##                 _map.loadFromMapFile(w[-2], folder,
##                                      name)
...
if __name__=='__main__':
    ...
    emap = mf.getMap('e')
    assert 'e' in list(mf._maps.keys())
