...
def autoroot(atoms):
    """
    for a ProDy atom set find the the root of he fleibility tree
    """
    ...
    for atom1 in atoms:
        ...
        for atom2 in atom1.iterBonded():
            thistree = mol.subTree(atom1, atom2)
...
def setADElements(atoms, obmol=None):
    ...
    if 'AD_element' not in list(ag._data.keys()):
        ag.setData('AD_element', ag.getElements())
    else:
        atoms.setData('AD_element', atoms.getElements())

    for atom in ob.OBMolAtomIter(obmol):
        # index returned by GetIndex is of type 'numpy.int64' - need to convert it to int in Python 3 to be able to use it with ag[index]
        index = int(obmol.obToProdyIndex[atom.GetIndex()])
