...
class MSMSParser:
    ...
    def getFaces(self, faceDescr):
        ...
        if faceDescr[0][0]=='#':
            ...
            facesComments = []
        facesInfo = [x.split() for x in facesLines]
        self.faces = numpy.array([[int(x[0])-1, int(x[1])-1,
                                                  int(x[2])-1] for x in facesInfo]).astype('i')
    ...
    def getVert(self, vertDescr):
        ...
        if vertDescr[0][0]=='#':
            ...
            vertComments = []
            vertLines = vertDescr
        vertInfo = [x.split() for x in vertLines]
        self.vertices = numpy.array(
            [[float(x[0]), float(x[1]), float(x[2])] for x in vertInfo]).astype('f')
        self.normals = numpy.array([[float(x[3]), float(x[4]), float(x[5])] for x in vertInfo]
                                     ).astype('f')
