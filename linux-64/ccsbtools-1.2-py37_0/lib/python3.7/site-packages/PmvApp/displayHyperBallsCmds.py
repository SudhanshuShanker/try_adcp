...
class DisplayHyperBalls(DisplayCommand):
    ...
    def onAddCmdToApp(self,):
        ...
        if 'assignAtomsRadii' not in self.app().commands:
...
#                if sof: setOff += sof
    ...
    def checkArguments(self, nodes, only=False, negate=False, 
        ...
        kw = {}
        assert isinstance(scaleFactor, Number)
        assert isinstance(cpkRad, Number)
        assert isinstance(quality, INT)
        assert isinstance(shrink, Number)
        assert isinstance(bScale, Number)
    ...
    def getProperties(self, selection):
        ...
        properties = [x[0] for x in list(selection[0].__dict__.items()) if (
            x[0][0]!='_' and isinstance(x[1], Number))]
...
class UndisplayHyperBalls(DisplayCommand):
    ...
    def onAddCmdToApp(self):
        DisplayCommand.onAddCmdToApp(self)
        if 'displayHyperBalls' not in self.app().commands:
