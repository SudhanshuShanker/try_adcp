    ...
    def __call__(self, path, pathNV, pathBNV, ptsPerRes, atIndices, extrudeVec,
        ...
        for i, pt in enumerate(path):
                ...
                if curSst!=sst: # new SS ELEMENT
                    ...
                    if i > 0:
                        ...
                        caps.append( (shape, extrudedC[-shapeLen::stride],
                                      list(range(nbExtV-shapeLen,nbExtV,stride)),
                    ...
                    if i>0:
                        ...
                        caps.append( (shape, tc[::stride],
                                      list(range(nbExtV-shapeLen,nbExtV, stride)), flipFaces, atind) )
                else:
                    ssElemChanged = False
            ...
            if i==0:
                ...
                # add cap for begining new shape
                nbExtV = len(extrudedC)
                caps.append( (shape, tc[::stride], list(range(nbExtV-shapeLen,nbExtV,stride)),
            ...
            vnum += shapeLen
        # add last cap
        caps.append( (shape, tc[::stride], list(range(nbExtV-shapeLen,nbExtV,stride)), not flipFaces, atind) )
        ...
        for shape, verts ,vind, flip, atind in caps:
            ...
            faces4At[atind].extend(list(range(fnum+foff, fnum+foff+len(f))))
            faces.extend(f)
            fnum += len(f)
