        ...
        if result==QtGui.QDialog.Accepted:
            ...
            if isinstance(object0, PmvSelection):
                ...
                for name in names:
                    ...
                    if name in list(self.pmvGUI.pmv.namedSelections.keys()) or name=='Current Selection' or \
...
class FetchGUI(QtGui.QDialog):
    ...
    def __init__(self, parent=None, formats=["pdb", "mmtf"]):
        ...
        self.kwEditW.returnPressed.connect(self.showTableDialog)
        ...
        hLayout.addWidget(self.kwEditW)
        hLayout.addWidget(self.searchButton)

        layout = QtGui.QFormLayout()
        layout.addRow("Pdb ID:", self.pdbidEditW)
        layout.addRow("keyword:", hLayout)
    ...
    def showTableDialog(self):
        ...
        kw = self.kwEditW.text()
        if not len(kw) : kw = None
        dialog = TableWidgetDialog("Search PDB", parent=self, keyword=kw)
        if kw is not None:
            dialog.fillTableDialog()
        result = dialog.exec_()
...
<accessionIdList>%s</accessionIdList>
    ...
    def fillTableDialog(self):
        ...
        if len(keyword):
            import _thread
            _thread.start_new_thread(self.keywordSearch, (keyword,) )
    ...
    def keywordSearch(self, keyword):
        ...
        if match is None:
            req = urllib.request.Request(self.url, data=self.txtSearch%(keyword, keyword))
        else:
            req = urllib.request.Request(self.url, data=self.uniprotACSearch%(keyword, keyword))
    ...
    def openFile(self, filenames=None):
        ...
        if filenames:
            ...
            for i, filename in enumerate(filenames):
                ...
                filenames[i] = str(filename.encode('ascii', 'replace'), "utf-8")
...
"""
        ...
        for obj, values in list(pick.hits.items()):
            # build a list of vertices and list of instances
            vertInds, instances = list(zip(*values))
            ...
            if obj.name in gc.geomPickToAtoms:
    ...
    def findPickedBonds(self, pick):
        ...
        for o, val in list(pick.hits.items()): #loop over geometries
            ...
            for instvert in val:
                ...
                if o.name in gc.geomPickToBonds:
...
"""
    ...
    def pickMenu(self, x, y, dx, dy, e):
        ...
        if len(pick.hits):
            ...
            if atoms.nbAtoms():
                ...
                for selSet in list(self.pmv.namedSelections.values()):
            ...
            if geom in cmdDict:
                ...
                for sele in selection:
                    ...
                    if not hasattr(mol.geomContainer, "msms"):
                        ...
                        if len(lastValues)==0: # surface was not computed yet
                            ...
                            for k,v in list(lastValues.items()):
                                nv = kw.get(k, None)
                                if nv!=v:
                ...
                for sele in selection:
                    ...
                    if not hasattr(mol.geomContainer, "boundGeom"):
                        recompute = True
                    elif surfName not in mol.geomContainer.boundGeom:
                        recompute = True
                    else:
                        try:
                ...
                for sele in selection:
                    ...
                    if mol.geomContainer.geoms.get(geom, None):
                        ...
                        if 'labelPlacement' in kw:
                            labPlacement = kw.pop('labelPlacement')
                            recompute = True
                            if 'mode' in labgeom.params:
    ...
    def buildMenuForObject(self, objItems, level=None, parent=None, parentMenu=None,
        ...
        if item0 is None: # use the molecule's root item
            try:
                item0 = list(object0[0].getAtomGroup().getMolecule()._treeItems.keys())[0]
        ...
        if selected.nbAtoms():
            ...
            if AddDeselected is True:
                ...
                if dynamicContextMenu:
                    for menuName, whatList in list(selectionTypes.items()):
                        targets = []
                        nb = 0
        ...
        if selected.nbAtoms() and (seleCount or curSelInTree):
            ...
            for name, sel in list(self.pmv.namedSelections.items()):
        ...
        if isinstance(molFrag, PmvSelection):
            ...
            for ats in atoms:
                ...
                if molFrag is self.pmv.curSelection:
                    ...
                    selection = ats
                gc = mol.geomContainer
                if surfName not in gc.geoms or len(ats)!=len(gc.atoms[surfName]):
                    recompute = True
                if recompute:
    ...
    def highlightSelection(self, event=None):
        ...
        for mol in app.Mols:
            ...
            for geomName, lGeom in list(geomC.geoms.items()):
            ...
            for geomName, lGeom in list(geomC.geoms.items()):
                if isinstance(lGeom, Spheres) \
                  or isinstance(lGeom, Cylinders):
                    ...
                    if len(highlight):
                        ...
                        if geomName == "doubleBondsSticks":
                            ...
                            if aromaticArcs is not None and len(aromaticArcs):
                                aindices = list(aromaticArcs.keys())
                                arinds = numpy.intersect1d(indices, aindices)
                                for index in arinds:
                                    if index in aromaticArcs:
                    ...
                    if hasattr(lGeom,'_isMSMS'):
                        ...
                        if len(lAtomSet) > 0:
                            ...
                            if len(highlight):
                                ...
                                if globProp['components']=='all':
                                    compIndices = list(range(srf.rsr.nb))
                                else:
                    ...
                    elif hasattr(geomC, "boundGeom") and geomName in geomC.boundGeom:
                        ...
                        if len(lSelectedAtoms):
                            ...
                            for i, at  in enumerate(cl_atoms):
                                if at not in lAtomVerticesDict:
...
#        self.windowsMenu.addAction(self.toggleCmdlineViewAct)
    ...
    def getRecentFilesMenu(self, submenu):
        ...
        for category, files in  list(self.pmv.recentFiles.categories.items()):
...
#            kernel.shell
    ...
    def reparentGroupObject(self, obj, group):
        ...
        if hasattr(obj, '_treeItems'):
            objItem = list(obj._treeItems.keys())[0]
    ...
    def renameGroupEventHandler(self, event):
        ...
        item = event.item
        if item is None:
            item = list(obj._treeItems.keys())[0]
    ...
    def afterDeleteAtomsEventHandler(self, event):
        ...
        for atom in event.objects:
            if hasattr(atom, '_treeItems'):
                for rootItem, item in [list(atom._treeItems.keys())[0]]:
        ...
        parentItems = {}
        for node in list(roots.keys()):
            if len(node.children)==0:
                for it in list(node._treeItems.values()):
                    parent = it.parent()
                    if parent:
    ...
    def renameTreeNodeEventHandler(self, event):
        ...
        if obj.alias:
            ...
            newName = obj.name
        if hasattr(obj, '_treeItems'):
            list(obj._treeItems.keys())[0].setText(0, newName)
    ...
    def deleteNamedSelectionEventHandler(self, event):
        ...
        if hasattr(sele, '_treeItems'):
            self.objTree.invisibleRootItem().removeChild(list(sele._treeItems.keys())[0])
    ...
    def activeSelectionChangedEventHandler(self, event):
        ...
        if hasattr(new, '_treeItems'):
            self.objTree.setCurrentSelection(list(new._treeItems.keys())[0])
