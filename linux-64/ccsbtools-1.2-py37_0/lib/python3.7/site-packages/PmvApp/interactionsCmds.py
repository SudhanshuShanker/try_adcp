...
class ProtonateWithReduce(ProtonateBase):
    ...
    def doit(self, molSel, flip=True, redraw=True):
        ...
        for name in list(mol._colors.keys()):
            if name=='msms':
               for msname in list(mol._colors['msms'].keys()):
        ...
        # add bonds
        addedAtoms = Selection(mol._ag, list(range(len(mol._ag)-len(agH),len(mol._ag))), '')
...
"""
    ...
    def updateSelections(self, mol, molH):
        ...
        for name, selSet in list(pmv.namedSelections.items()):
